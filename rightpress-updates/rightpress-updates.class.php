...
final class RightPress_Updates_7119279
{
    ...
    // Version number
    private static $rightpress_updates_version = '2.3';
    ...
    public function __construct($plugin_path, $plugin_version)
    {
        ...
        $this->endpoint_url     = 'https://updates.rightpress.net/';
        ...
        add_action('admin_init', array($this, 'on_wp_init'), 1);
    }
    ...
    public function print_nag($context, $nag_version, $is_new_version)
    {
        ...
        if ($context === 'up') {
            ...
            // Format
            $notes = '<a href="https://url.rightpress.net/purchase-code-help">' . esc_html__('Where do I find my Purchase Code?', 'rightpress-updates') . '</a>';
            $notes .= '&nbsp;&nbsp;&nbsp;';
        ...
        // Print nag
        echo '<div class="rightpress-clear-both"></div><div class="update-nag rightpress-updates-update-nag"><h3>' . esc_html($title) . '</h3>' . $content . '<div class="rightpress-clear-both"></div></div>';
    }
    ...
    public function maybe_display_activation_information_inline($args)
    {
        ...
        if (empty($this->purchase_code)) {
            echo ' <em>' . sprintf(esc_html__('You must enter your Purchase Code %s to enable automatic updates.', 'rightpress-updates'), ('<a class="rightpress_up_pc_' . $this->plugin_key . '" href="#">' . esc_html__('here', 'rightpress-updates') . '</a>')) . ' <a href="https://url.rightpress.net/purchase-code-help">' . esc_html__('Where do I find my Purchase Code?', 'rightpress-updates') . '</a></em>';
        }
        // Support expired
        else if (get_site_option('rightpress_updates_expired_' . $this->plugin_key)) {
            echo ' <em>' . sprintf(esc_html__('RightPress no longer serves automatic updates as your %s has expired.', 'rightpress-updates'), ('<a href="https://url.rightpress.net/automatic-updates-help">' . __('support subscription', 'rightpress-updates') . '</a>')) . ' ' . sprintf(esc_html__('You can %1$s your support subscription or use %2$s to keep this plugin updated.', 'rightpress-updates'), ('<a href="https://url.rightpress.net/' . self::$envato_id . '-product">' . esc_html__('renew', 'rightpress-updates') . '</a>'), ('<a href="https://url.rightpress.net/updates-help">' . esc_html__('other methods', 'rightpress-updates') . '</a>')) . '</em>';
        }
    }
