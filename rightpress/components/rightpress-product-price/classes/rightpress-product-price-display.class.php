...
final class RightPress_Product_Price_Display
{
    ...
    public function __construct()
    {
        ...
        // Is on sale fix
        RightPress_Help::add_early_filter('woocommerce_product_is_on_sale', array($this, 'product_is_on_sale_fix'), 2);
    }
...
// TODO: Do we need to handle all of these?
     ...
     * @return bool
     */
    public function product_is_on_sale_fix($on_sale, $product)
    {

        // TBD: This does not yet contain sale date checks (see issue #866)

        // Product is not simple or variation
        ...
        if ($on_sale) {
            ...
            // Double check to make sure WooCommerce check is still true (in case third-parties already modified this result)
            if ('' !== (string) $sale_price && $regular_price > $sale_price) {

                // Get decimals
                $decimals = wc_get_price_decimals();

                // Sale price is no longer lower than regular price if we round prices to decimal precision
                if (round($regular_price, $decimals) <= round($sale_price, $decimals)) {
