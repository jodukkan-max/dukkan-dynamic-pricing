...
final class RightPress_Product_Price_Changes
{
    ...
    private static function post_callback_cart_item_price_changes_processing(&$cart_item_changes, $cart_items, $cart_item_key)
    {
        ...
        foreach ($cart_item_changes['prices']['ranges'] as $price_range_key => $price_range) {
            ...
            // Also lift marked-up regular price into highest price (set by percentage fee markup methods)
            if (isset($price_range['regular_price']) && $price_range['regular_price'] !== null && $price_range['regular_price'] > $price_range['highest_price']) {
                $cart_item_changes['prices']['ranges'][$price_range_key]['highest_price'] = $price_range['regular_price'];
            }
        }
    }
    ...
    public static function get_highest_price_from_cart_item_price_changes($price_data, $price_range = array())
    {
        ...
        // Include marked-up regular price as a candidate when available (set by percentage fee markup methods)
        if (!empty($price_range) && isset($price_range['regular_price']) && $price_range['regular_price'] !== null) {
            $highest_price_alternatives[] = $price_range['regular_price'];
        }
