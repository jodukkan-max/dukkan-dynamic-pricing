...
final class RightPress_Product_Price_Test
{
    ...
    public static function run($product, $quantity = 1, $variation_attributes = array(), $return_data = false, $return_empty_changes = false, $cart_item_data = array(), $custom_price = null, $use_empty_cart = false)
    {
        ...
        if ($instance->product_flagged) {
            RightPress_Product_Price::unset_product_in_cart($product);
        ...
        try {
            ...
            foreach ($cart_items_to_add as $cart_item_to_add) {
                ...
                // Item not yet in cart
                    ...
                    RightPress_Product_Price::set_product_in_cart($product, $cart_item_key);
        ...
        foreach ($cart_items_price_changes as $cart_item_price_changes) {
            ...
            foreach ($cart_item_price_changes['prices']['ranges'] as $price_range) {
                ...
                $price_changes['prices']['ranges'][('to_' . $to)] = array(
                    ...
                    'regular_price' => (isset($price_range['regular_price']) ? $price_range['regular_price'] : null),
