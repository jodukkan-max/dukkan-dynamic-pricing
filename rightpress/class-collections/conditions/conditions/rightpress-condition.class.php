            ...
            if ($field = $condition_fields_controller->get_item($field_key)) {
                if (isset($params['condition'][$field_key])) {
                    ...
                    if ($field->supports_hierarchy()) {
                        ...
                        // Get value with children
                        $value_with_children = $field->get_children($value);

                        // Remove empty values (e.g. parent product category can be deleted)
                        $value_with_children = array_filter($value_with_children);

                        // Return value with children
