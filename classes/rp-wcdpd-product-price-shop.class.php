...
class RP_WCDPD_Product_Price_Shop
{
    ...
    public function init()
    {
        ...
        // Maybe skip temporary cache for this request
        add_filter('rightpress_product_price_shop_skip_temporary_cache', array($this, 'maybe_skip_temporary_cache'), $this->rightpress_hook_position, 2);
    ...
    public function calculate_price($calculation_data, $price_type, $product)
    {
        ...
        // Percentage fee/discount handling for simple rules
        $percentage_prices = false;
        if (in_array(RP_WCDPD_Settings::get('product_pricing_change_display_prices'), array('change_simple', 'change_all'), true)) {
            $percentage_prices = RP_WCDPD_Product_Pricing::apply_percentage_rules_to_product($product);
        }

        // Calculate price
        if ($price_type === 'price') {

            // Percentage rules applied - use computed sale price (or marked-up regular price if no discount)
            if ($percentage_prices) {
                $adjusted_price = ($percentage_prices['sale_price'] !== null) ? $percentage_prices['sale_price'] : $percentage_prices['regular_price'];
            }
            // Get adjusted price
            else {
        ...
        // Calculate sale price
            ...
            // Percentage rules applied - use computed sale price (empty if no discount rule)
            if ($percentage_prices) {
                if ($percentage_prices['sale_price'] !== null) {
                    $adjusted_price = $percentage_prices['sale_price'];
                }
                else {
        ...
        // Calculate regular price
            ...
            // Percentage rules applied - use computed marked-up regular price
            if ($percentage_prices) {
                $adjusted_price = $percentage_prices['regular_price'];
            }
            // Regular price may not need to be displayed
            else if (!RP_WCDPD_Settings::get('product_pricing_display_regular_price')) {

                // Check if sale price is not set in WooCommerce product settings
    ...
    public function add_settings_hash_data($hash_data, $deprecated_1)
    {
        ...
        $hash_data['rp_wcdpd'] = array(
            ...
            // Version marker: percentage fee marks up regular price, percentage
            // discount creates sale price from it. Bump to invalidate cached shop
            // prices when this behaviour changes.
            'rp_wcdpd_percentage_regular_anchor_v1',
        );
     ...
     * @return bool
     */
    public function maybe_skip_temporary_cache($skip, $product)
    {
