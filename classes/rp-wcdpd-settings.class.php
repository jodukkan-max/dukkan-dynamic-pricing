...
class RP_WCDPD_Settings
{
                        ...
                        'import_export' => array(
                            ...
                            'children' => array(
                                ...
                                'export' => array(
                                    ...
                                    'link_url'      => wp_nonce_url(admin_url('?rp_wcdpd_export_settings=1'), 'rp_wcdpd_export_settings'),
                                ),
                            ),
                        ),
                    ),
            ...
            foreach ($input[$rule_type] as $posted) {
                ...
                if (in_array($rule_type, array('cart_discounts', 'checkout_fees'), true)) {
                    if (isset($posted['title']) && !RightPress_Help::is_empty($posted['title'])) {
                        $current['title'] = sanitize_text_field((string) stripslashes($posted['title']));
                    }
                    else {
                        continue;
                    }
                ...
                if (isset($posted['note']) && !RightPress_Help::is_empty($posted['note'])) {
                    $current['note'] = sanitize_textarea_field((string) stripslashes($posted['note']));
                }
                else {
                    $current['note'] = '';
                }
                ...
                if (isset($posted['public_note']) && !RightPress_Help::is_empty($posted['public_note'])) {
                    $current['public_note'] = sanitize_textarea_field((string) stripslashes($posted['public_note']));
                }
                else {
                    $current['public_note'] = '';
                }
    ...
    public function maybe_display_migration_notice()
    {
        ...
        if ($notice = get_option('rp_wcdpd_migration_notice')) {
            ...
            // Print
            printf('<div class="update-nag rp_wcdpd_migration_notice"><h3>Action Required!</h3>' . $notice . '<p><a href="%s">Contact Support</a>&nbsp;&nbsp;&nbsp;<a href="%s">Hide this notice</a></p></div>', 'https://url.rightpress.net/new-support-ticket', add_query_arg('rp_wcdpd_hide_migration_notice', '1'));
        ...
        if ($notice = get_option('rp_wcdpd_migration_notice_products_to_adjust')) {
            ...
            // Print
            printf('<div class="update-nag rp_wcdpd_migration_notice"><h3>Warning!</h3>' . $notice . '<p><a href="%s">Contact Support</a>&nbsp;&nbsp;&nbsp;<a href="%s">Hide this notice</a></p></div>', 'https://url.rightpress.net/new-support-ticket', add_query_arg('rp_wcdpd_hide_migration_notice_products_to_adjust', '1'));
        ...
        if ($notice = get_option('rp_wcdpd_migration_notice_2021_09_security')) {
            ...
            // Print
            printf('<div class="update-nag rp_wcdpd_migration_notice"><h3>Warning!</h3>' . $notice . '<p><a href="%s">Find Out More</a>&nbsp;&nbsp;&nbsp;<a href="%s">Contact Support</a>&nbsp;&nbsp;&nbsp;<a href="%s">Hide this notice</a></p></div>', 'https://url.rightpress.net/wcdpd-security-announcement', 'https://pricepep.com/support/', add_query_arg('rp_wcdpd_hide_migration_notice_2021_09_security', '1'));
    ...
    public function export()
    {
        ...
        // Verify nonce
        check_admin_referer('rp_wcdpd_export_settings');
