...
class RP_WCDPD_WC_Cart
{
    ...
    public function maybe_invalidate_regular_coupon($is_valid, $coupon)
    {
        ...
        if (!RP_WCDPD_Controller_Methods_Cart_Discount::coupon_is_cart_discount($coupon_code)) {
            ...
            // Regular coupons are not allowed with cart discounts and cart contains cart discounts
            if (!RP_WCDPD_Settings::get('cart_discounts_allow_coupons') && RP_WCDPD_Controller_Methods_Cart_Discount::cart_contains_cart_discount()) {
            ...
            // Regular coupons are not allowed with product pricing adjustments and cart contains product pricing adjustments
            else if (!RP_WCDPD_Settings::get('product_pricing_allow_coupons') && RightPress_Product_Price_Cart::get_cart_item_price_changes()) {

                // Do not allow regular coupon
                // Note: using a random error code to make sure we don't match any other error codes accidentally
                throw new Exception('4582405412');
            }
        }
    ...
    public function invalidated_regular_coupon_error($error_message, $error_code, $coupon)
    {
        ...
        else if ($error_code === '4582405412') {
            $error_message = sprintf(esc_html__('Sorry, coupon "%s" is not valid when cart item prices are changed.', 'rp_wcdpd'), $coupon->get_code());
        }
