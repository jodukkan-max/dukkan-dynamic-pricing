...
class RP_WCDPD_Product_Pricing
{
    ...
    /**
     * Apply percentage fee and percentage discount simple rules to a product
     *
     * Percentage fee rules mark up the regular price, percentage discount rules
     * create a sale price from the current regular price (marked up by fee rules
     * if any). Anchors to the product's original regular price and ignores any
     * pre-existing WooCommerce sale price.
     *
     * @access public
     * @param object $product
     * @return array|bool
     */
    public static function apply_percentage_rules_to_product($product)
    {
        ...
        if (is_array($applicable_rules) && !empty($applicable_rules)) {
            ...
            // Anchor to the product's original regular price
            $regular_price = (float) $product->get_regular_price('edit');
            $sale_price = null;
            $has_fee = false;
            $has_discount = false;

            // Apply percentage fee (markup) rules first
            foreach ($applicable_rules as $applicable_rule) {
                $pricing_method = isset($applicable_rule['pricing_method']) ? $applicable_rule['pricing_method'] : null;
                if ($pricing_method === 'fee__percentage') {
                    $regular_price = RP_WCDPD_Pricing::adjust_amount($regular_price, $pricing_method, $applicable_rule['pricing_value']);
                    $has_fee = true;
                }
            }

            // Then apply percentage discount (markdown) rules to create a sale price
            foreach ($applicable_rules as $applicable_rule) {
                $pricing_method = isset($applicable_rule['pricing_method']) ? $applicable_rule['pricing_method'] : null;
                if ($pricing_method === 'discount__percentage') {
                    $sale_price = RP_WCDPD_Pricing::adjust_amount($regular_price, $pricing_method, $applicable_rule['pricing_value']);
                    $has_discount = true;
                }
            }

            // Return computed prices if any percentage rule was applied
            if ($has_fee || $has_discount) {
                return array(
