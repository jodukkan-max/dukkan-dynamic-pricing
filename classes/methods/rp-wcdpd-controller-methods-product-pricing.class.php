...
class RP_WCDPD_Controller_Methods_Product_Pricing extends RP_WCDPD_Controller_Methods
{
    ...
    public function price_changes_applied_to_cart_item($price, $cart_item_key, $cart, $price_change)
    {
        ...
        if (!empty($price_change['new_changes']['rp_wcdpd'])) {
            ...
            foreach ($price_change['new_changes']['rp_wcdpd'] as $current_change) {
                ...
                if (!RP_WCDPD_Settings::get('product_pricing_allow_coupons')) {
    ...
    private function apply_adjustments_to_prices_of_cart_item($prices, $cart_item, $cart_item_key, $cart_item_adjustments, $test_cart_items = array())
    {
        ...
        // Order adjustments so percentage fee (markup) rules run before
        // percentage discount (markdown) rules, so the markdown is always
        // computed from the marked-up regular price
        $fee_adjustments      = array();
        $discount_adjustments = array();
        $other_adjustments    = array();

        foreach ($cart_item_adjustments as $rule_uid => $adjustment) {
            $pricing_method = isset($adjustment['rule']['pricing_method']) ? $adjustment['rule']['pricing_method'] : null;
            if ($pricing_method === 'fee__percentage') {
                $fee_adjustments[$rule_uid] = $adjustment;
            }
            else if ($pricing_method === 'discount__percentage') {
                $discount_adjustments[$rule_uid] = $adjustment;
            }
            else {
                $other_adjustments[$rule_uid] = $adjustment;
            }
        }

        // Combine ordered adjustments
        $cart_item_adjustments = $fee_adjustments + $discount_adjustments + $other_adjustments;
