...
class RP_WCDPD_Controller_Methods_Cart_Discount extends RP_WCDPD_Controller_Methods
{
    ...
    public function __construct()
    {
        ...
        // Change virtual coupon label in classic cart/checkout (blocks handled via JS)
        ...
        // Pass virtual coupon data via Store API (cart/checkout blocks)
        add_action('woocommerce_blocks_loaded', function() {
            if (function_exists('woocommerce_store_api_register_endpoint_data')) {
                woocommerce_store_api_register_endpoint_data(array(
                    'endpoint'          => 'cart',
                    'namespace'         => 'rp_wcdpd_virtual_coupon_data',
                    'data_callback'     => [$this, 'get_virtual_coupon_data_for_api'],
                    'schema_callback'   => [$this, 'get_virtual_coupon_data_schema'],
                    'schema_type'       => ARRAY_A,
                ));
            }
        });
    ...
    /**
     * Get virtual coupon data for Store API (cart/checkout blocks)
     *
     * @access public
     * @return array
     */
    public function get_virtual_coupon_data_for_api()
    {
        ...
        // Loop through all applied coupons in the cart
        foreach (WC()->cart->get_coupons() as $coupon_code => $coupon) {

            // Coupon is cart discount
            if (RP_WCDPD_Controller_Methods_Cart_Discount::coupon_is_cart_discount($coupon_code)) {

                // Get label
                $data['labels'][$coupon_code] = $this->change_virtual_coupon_label($coupon_code, $coupon);
            }
        }
    ...
    /**
     * Define date type for Store API
     *
     * @access public
     * @return array
     */
    public function get_virtual_coupon_data_schema()
    {
        ...
        return [
            ...
            'properties'    => [
                'labels'    => [
                    'type'                  => 'object',
                    'description'           => 'Map of coupon codes to labels',
